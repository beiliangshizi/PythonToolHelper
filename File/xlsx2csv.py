...
class Xlsx2csv:
        ...
        if sheetid > 0:
            ...
            for s in self.workbook.sheets:
                ...
                if isinstance(outfile, str):
                    ...
                    of.write(self.options['sheetdelimiter'] + " " + str(s['id']) + " - " + sheetname + self.options[
                        'lineterminator'])
    ...
    def _convert(self, sheetid, outfile):
        ...
        if isinstance(outfile, str):
            ...
            closefile = True
        try:
            writer = csv.writer(outfile, quoting=csv.QUOTE_MINIMAL, delimiter=self.options['delimiter'],
    ...
    def parse(self, filehandle):
        ...
        if len(fileVersion) == 0:
            ...
            try:
                if workbookDoc.firstChild.namespaceURI:
                    self.appName = \
                    workbookDoc.firstChild.getElementsByTagNameNS(workbookDoc.firstChild.namespaceURI, "fileVersion")[
                        0]._attrs['appName'].value
                else:
        ...
        try:
            if workbookDoc.firstChild.namespaceURI:
                self.date1904 = \
                workbookDoc.firstChild.getElementsByTagNameNS(workbookDoc.firstChild.namespaceURI, "workbookPr")[
                    0]._attrs['date1904'].value.lower().strip() != "false"
            else:
                        ...
                        if format_type == 'date':  # date/time
                            ...
                            if self.dateformat:
                                ...
                                dateformat = re.sub(r"\[\$\-\d{1,3}\]", "", format_str, 1). \
                                    ...
                                    replace("hh:mm", "%H:%M").replace("h", "%I").replace("%H%H", "%H").replace("ss",
                                                                                                               "%S"). \
